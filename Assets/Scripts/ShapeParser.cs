...
namespace MercenariesProject
{
    ...
    public class ShapeParser
    {
        ...
        public List<Tile> GetAbilityTileLocations(Tile abilityPosition, TextAsset defaultAbility, Vector2Int characterPosition)
        {
            ...
            foreach (var location in affectedTileLocations)
            {
                if (grid.ContainsKey(location))
                    affectedTiles.Add(grid[location]);
            }
        ...
        private List<Vector2Int> GetShapeAbilityTiles(Tile abilityTile, Vector2Int characterPos, int[,] abilityMap, List<Vector2> originLocation)
        {
            List<Vector2> abilityAffectedTiles = new List<Vector2>();
            var abilityPosition = abilityTile.gridLocation;
        ...
        private List<Vector2Int> GetLineAbilityTiles(Tile abilityTile, Vector2Int characterPosition, int[,] abilityMap, List<Vector2> originLocation)
        {
            List<Vector2> abilityAffectedTiles = new List<Vector2>();
            var abilityPosition = abilityTile.gridLocation;
        ...
        public List<Vector2Int> GetAffectedTilesInALine(ref Vector2Int originPosition, Vector2Int tileOffset, Vector2Int charpos)
        {
            ...
            int xBoundry = gridSize[0];
            int yBoundry = gridSize[1];
            if (absDirectionFromActiveCharacter.x >= absDirectionFromActiveCharacter.y)
            {
                if (directionFromActiveCharacter.x <= 0)
                {
                    ...
                    float lenght = Mathf.Abs(-positions[0].x) + 1;
                ...
                {
                    positions.Add(new Vector2Int(originPosition.x + tileOffset.x, originPosition.y + tileOffset.y));
                    float lenght = Mathf.Abs(xBoundry - positions[0].x);
            ...
            else
            {
                if (directionFromActiveCharacter.y <= 0)
                {
                    ...
                    float lenght = Mathf.Abs(- positions[0].y);
                ...
                else
                {
                    ...
                    float lenght = Mathf.Abs(yBoundry - positions[0].y) + 1;
